...
/*
 * V0084: shutdown receive direction only.
 *
 * Spec 5.10.6.6: SHUTDOWN with only VIRTIO_VSOCK_SHUTDOWN_RCV set
 * closes the receive direction while leaving send open. The device
 * must accept this partial shutdown.
 */
...
static test_result_t test_vsock_shutdown_recv(struct virtio_dev *dev,
                                             struct vring *vr)
{
    /* CONNECT */
    struct virtio_vsock_hdr *conn = vv_alloc_pages(1);
    conn->src_cid = 3; conn->dst_cid = 2;
    conn->src_port = 1100; conn->dst_port = 1100;
...
REGISTER_TEST(V0084, VIRTIO_PCI_DEVICE_VSOCK, test_vsock_shutdown_recv,
              "Shutdown receive direction only",
