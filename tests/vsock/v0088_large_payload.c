...
/*
 * V0088: stream RW with large (4096 byte) payload.
 *
 * Send CONNECT then RW with 4096 bytes of data. Tests the device
 * handles a full page payload on an established connection.
 */
...
static test_result_t test_vsock_large(struct virtio_dev *dev, struct vring *vr)
{
    struct virtio_vsock_hdr *conn = vv_alloc_pages(1);
    conn->src_cid = 3; conn->dst_cid = 2;
    conn->src_port = 7700; conn->dst_port = 7700;
    ...
    rw->src_port = 7700; rw->dst_port = 7700;
    rw->len = 4096; rw->type = VIRTIO_VSOCK_TYPE_STREAM;
    rw->op = VIRTIO_VSOCK_OP_RW;
    rw->flags = 0; rw->buf_alloc = 65536; rw->fwd_cnt = 0;
    memset(payload, 0x42, 4096);

    vring_raw_set_desc(vr, 0, vv_virt_to_phys(rw), sizeof(*rw),
                       VRING_DESC_F_NEXT, 1);
    vring_raw_set_desc(vr, 1, vv_virt_to_phys(payload), 4096, 0, 0);
...
REGISTER_TEST(V0088, VIRTIO_PCI_DEVICE_VSOCK, test_vsock_large,
              "Stream RW with 4096 byte payload",
