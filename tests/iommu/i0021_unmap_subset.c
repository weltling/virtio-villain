...
/*
 * I0021: IOMMU UNMAP a strict subset of a mapped range.
 *
 * MAP [0x10000, 0x10FFF] then UNMAP [0x10400, 0x107FF]. The
 * device may either split the existing mapping or reject the
 * partial unmap, but it must not crash and must not leave the
 * range table in an inconsistent state.
 *
 * Spec 5.13.5.7.
 */
...
static test_result_t test_iommu_unmap_subset(struct virtio_dev *dev,
                                             struct vring *vr)
{
...
REGISTER_TEST(I0021, VIRTIO_PCI_DEVICE_IOMMU, test_iommu_unmap_subset,
              "Unmap a subset of a mapped range",
