...
/*
 * I0046: IOMMU UNMAP that exactly removes the existing range.
 *
 * MAP [0x10000, 0x10FFF] then UNMAP the identical range. Per
 * spec 5.13.5.7 the device must remove the mapping and return
 * VIRTIO_IOMMU_S_OK. Subsequent translations of the range fail.
 *
 * Spec 5.13.5.7.
 */
...
static test_result_t test_iommu_unmap_exact(struct virtio_dev *dev,
                                            struct vring *vr)
{
...
REGISTER_TEST(I0046, VIRTIO_PCI_DEVICE_IOMMU, test_iommu_unmap_exact,
              "Unmap removes the exact mapped range",
