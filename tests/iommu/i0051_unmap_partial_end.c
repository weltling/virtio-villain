...
/*
 * I0051: UNMAP straddling the end of an existing mapping.
 *
 * MAP a range, then UNMAP a range whose virt_start lies inside
 * the mapping and whose virt_end lies past its end. Symmetric
 * counterpart to I0050. The device must not crash and the
 * range table must remain consistent.
 *
 * Spec 5.13.5.7.
 */
...
static test_result_t test_iommu_unmap_partial_end(struct virtio_dev *dev,
                                                  struct vring *vr)
{
    ...
    u->domain      = 0;
    u->virt_start  = 0xB1000;            /* inside mapping */
    u->virt_end    = 0xB2FFF;            /* past mapping end */
    u->tail.status = 0xFF;
...
REGISTER_TEST(I0051, VIRTIO_PCI_DEVICE_IOMMU, test_iommu_unmap_partial_end,
              "Unmap straddling the end of a mapping",
