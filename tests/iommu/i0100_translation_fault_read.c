...
/*
 * I0100: write only MAP cannot serve reads.
 *
 * v1.4 5.13.6.3: MAP with flags = WRITE only permits writes.
 * A real read transaction would fault. We exercise the
 * write only flag combination to verify the device accepts
 * it (rather than confusing flag bits).
 */
...
static test_result_t test(struct virtio_dev *dev, struct vring *vr)
{
    ...
    a->domain = 0xA001; a->endpoint = 0; a->tail.status = 0xFF;
    ...
    m->virt_end = 0xFFF;
    m->phys_start = 0;
    m->flags = VIRTIO_IOMMU_MAP_F_WRITE;
    m->tail.status = 0xFF;
...
REGISTER_TEST(I0100, VIRTIO_PCI_DEVICE_IOMMU, test,
              "MAP with WRITE only flag accepted",
