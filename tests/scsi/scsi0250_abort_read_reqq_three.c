...
/*
 * SCSI0250: abort_read_reqq_three
 *
 * A host side sidecar throttles the drive so a guest read on the
 * second request queue stays queued. An ABORT TASK by tag on the
 * control queue terminates it, confirming task management reaches a
 * request submitted on a non first request queue. Skips when the
 * throttle is not applied.
 */
#include "tests/scsi/scsi_util.h"

#define ABORT_TAG 0x50500001ULL

static test_result_t test_scsi_abort_reqq_three(struct virtio_dev *dev,
                                                struct vring *vr)
{
    struct vring rq;
    if (vring_alloc(&rq, 16) < 0)
        return TEST_SKIP;
    /* Second request queue (controlq 0, eventq 1, request queues 2..). */
...
REGISTER_TEST_Q(SCSI0250, VIRTIO_PCI_DEVICE_SCSI, test_scsi_abort_reqq_three,
                "ABORT TASK reaches a read on the second request queue",
