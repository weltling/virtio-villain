...
/*
 * SCSI0255: double_abort_same_tag
 *
 * A host side sidecar throttles the drive so a guest read stays
 * queued. A first ABORT TASK terminates it, then a second ABORT TASK
 * for the same tag finds no task and reports the function complete.
 * Skips when the throttle is not applied.
 */
#include "tests/scsi/scsi_util.h"

#define ABORT_TAG 0x55550001ULL

static test_result_t test_scsi_double_abort(struct virtio_dev *dev,
                                            struct vring *vr)
{
    struct vring rq;
    ...
    if (rb_resp->response != VIRTIO_SCSI_S_ABORTED)
        TFAIL("first abort response 0x%02x is not aborted", rb_resp->response);

    /* Second abort for the same tag: the task is gone. */
    struct virtio_scsi_ctrl_tmf_req *tmf2 = vv_alloc_pages(1);
    struct virtio_scsi_ctrl_tmf_resp *tmf2_resp = vv_alloc_pages(1);
    memset(tmf2, 0, sizeof(*tmf2));
    tmf2->type = VIRTIO_SCSI_T_TMF;
    tmf2->subtype = VIRTIO_SCSI_T_TMF_ABORT_TASK;
    scsi_set_lun(tmf2->lun, 0, 0);
    tmf2->tag = ABORT_TAG;
    scsi_do_tmf(dev, vr, tmf2, tmf2_resp, 1);

    if (tmf2_resp->response != VIRTIO_SCSI_S_FUNCTION_COMPLETE)
        TFAIL("second abort response 0x%02x is not function complete",
              tmf2_resp->response);
    return TEST_PASS;
}

REGISTER_TEST_Q(SCSI0255, VIRTIO_PCI_DEVICE_SCSI, test_scsi_double_abort,
                "A second ABORT TASK for a gone tag reports function complete",
