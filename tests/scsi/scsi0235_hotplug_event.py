...
"""
SCSI0235 sidecar. Hot adds a scsi-hd logical unit on the running
virtio-scsi controller so the device delivers a transport reset
event to the guest event queue. QEMU only. On other backends or
when the API socket is disabled the sidecar exits cleanly and the
guest test skips.
"""
import os
...
def run(ctx):
    ...
    ctx.log(f"created hot plug image {hp}")
