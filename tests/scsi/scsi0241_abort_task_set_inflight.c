...
/*
 * SCSI0241: abort_task_set_inflight
 *
 * A host side sidecar throttles the drive so a guest read stays
 * queued. An ABORT TASK SET on the control queue terminates the
 * pending task and the device completes the read with the aborted
 * response.
 *
 * Runs under QEMU with the API socket enabled. Skips when the
 * throttle is not applied.
 */
#include "tests/scsi/scsi_util.h"

static test_result_t test_scsi_abort_task_set(struct virtio_dev *dev,
                                              struct vring *vr)
{
    struct vring rq;
...
REGISTER_TEST_Q(SCSI0241, VIRTIO_PCI_DEVICE_SCSI, test_scsi_abort_task_set,
                "ABORT TASK SET terminates a throttled read",
