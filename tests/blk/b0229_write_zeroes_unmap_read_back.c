...
/*
 * B0229: write zeroes with unmap flag then read returns zeros.
 *
 * Seed sector 0 with a nonzero pattern, issue WRITE_ZEROES with the
 * UNMAP flag set, then read it back. Spec 5.2.6.2 states that
 * sectors zeroed with the unmap flag are still read back as zeros,
 * so the deallocation hint must not leak stale data.
 */
...
static test_result_t test_blk_wz_unmap_read_back(struct virtio_dev *dev,
                                                 struct vring *vr)
{
...
REGISTER_TEST_REQUIRES(B0229, VIRTIO_PCI_DEVICE_BLK, test_blk_wz_unmap_read_back,
              "Write zeroes with unmap flag then read returns zeros",
