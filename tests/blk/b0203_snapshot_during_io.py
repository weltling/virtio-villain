...
"""
B0203 sidecar. Snapshots the running VM while the guest is between
two reads. Snapshot files land in ctx.tmpdir.
"""
import os
...
def run(ctx):
    ...
    if cp.returncode != 0:
        ctx.log(f"snapshot failed rc={cp.returncode} "
