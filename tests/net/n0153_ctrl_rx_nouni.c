...
/*
 * N0153: ctrl RX no-unicast mode.
 *
 * Spec 5.1.6.5.1: When VIRTIO_NET_F_CTRL_RX is negotiated, send
 * CTRL_RX_NOUNI with data=1 (enable no-unicast filtering). Verify
 * VIRTIO_NET_OK.
 */
...
static test_result_t test_net_ctrl_nouni(struct virtio_dev *dev,
                                         struct vring *vr)
{
    ...
    hdr->command = VIRTIO_NET_CTRL_RX_NOUNI;
    *data = 1;
    *ack = 0xFF;
...
REGISTER_TEST_Q_REQUIRES(N0153, VIRTIO_PCI_DEVICE_NET, test_net_ctrl_nouni,
              "CTRL RX no-unicast enable",
