...
/*
 * N0148: ctrl RX allmulti enable.
 *
 * Spec 5.1.6.5.1: When VIRTIO_NET_F_CTRL_RX is negotiated the
 * driver may set all-multicast mode via CTRL_RX_ALLMULTI. Send
 * enable (data=1) and verify VIRTIO_NET_OK.
 */
...
static test_result_t test_net_ctrl_allmulti(struct virtio_dev *dev,
                                           struct vring *vr)
{
...
REGISTER_TEST_Q_REQUIRES(N0148, VIRTIO_PCI_DEVICE_NET, test_net_ctrl_allmulti,
              "CTRL RX all-multicast enable",
