...
/*
 * N0179: net rx writable descriptor whose addr plus len wraps 2^64.
 *
 * Siblings N0112 and the rx overflow tests keep the end address below
 * 2^64. Here the receive buffer base sits near the top of the address
...
static test_result_t test_net_rx_addr_len_wrap(struct virtio_dev *dev,
                                              struct vring *vr)
{
...
REGISTER_TEST(N0179, VIRTIO_PCI_DEVICE_NET, test_net_rx_addr_len_wrap,
              "Net rx writable descriptor addr plus len wraps 64 bits",
