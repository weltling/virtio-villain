...
/*
 * P0027: Packed used completion with wrong len field (spec 2.8.8)
 *
 * Submit a valid request, then verify the device sets the used
 * len field. This tests device compliance on used ring writeback.
 */
...
static test_result_t test_packed_used_len(struct virtio_dev *dev,
    ...
    *st = 0xFF;
    ...
    while (elapsed < VV_TIMEOUT_MS * 1000) {
        ...
        if (used_bit == avail_bit) {
            /* Descriptor marked as used - check len */
            uint32_t used_len = vr->desc[idx].len;
            /* Should be total writable bytes written: 512 + 1 = 513 */
            if (used_len > 513)
                TFAIL("used_len > 513"); /* len exceeds writable buffer */
            return TEST_PASS;
        }
        elapsed += 10000;
    }
...
REGISTER_TEST_PACKED(P0027, VIRTIO_PCI_DEVICE_BLK, test_packed_used_len,
    "Packed used completion len must not exceed writable buffer",
    VIRTIO_SPEC_V1_2, "2.8.8");
