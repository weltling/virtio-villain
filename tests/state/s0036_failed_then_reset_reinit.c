...
/*
 * S0036: FAILED then reset then reinit
 *
 * Spec 2.1.2 says the driver may indicate it has given up by
 * setting FAILED, and that a reset returns the device to its
 * initial state regardless of prior status. This test sets the
 * FAILED bit, then resets, then runs a full reinit ending in
 * valid I/O. A VMM that treats the prior FAILED as sticky across
 * reset will refuse to come back online.
 */
...
static test_result_t test_failed_then_reset_reinit(struct virtio_dev *dev,
                                                   struct vring *vr)
{
    ...
    /* FAILED must be clear after the reset path */
    if (cfg->device_status & VIRTIO_STATUS_FAILED)
...
REGISTER_TEST(S0036, VIRTIO_PCI_DEVICE_BLK, test_failed_then_reset_reinit,
              "FAILED bit cleared by reset then full reinit succeeds",
