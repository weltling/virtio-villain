...
/*
 * C0057: tx_indirect_max_entries
 *
 * With VIRTIO_F_INDIRECT_DESC negotiated a transmit request uses a
 * full indirect table of readable entries. The device must gather the
 * whole table without overrunning it.
 */
...
static test_result_t test_console_tx_indirect_max_entries(struct virtio_dev *dev,
                                                          struct vring *vr)
{
    ...
    for (int i = 0; i < ENTRIES; i++) {
        ...
        itab[i].len = 128;
        itab[i].flags = last ? 0 : VRING_DESC_F_NEXT;
...
REGISTER_TEST_Q_REQUIRES(C0057, VIRTIO_PCI_DEVICE_CONSOLE,
                         test_console_tx_indirect_max_entries,
                         "Console transmit indirect using a full entry set",
