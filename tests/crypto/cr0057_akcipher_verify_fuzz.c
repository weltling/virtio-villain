...
/*
 * CR0057: crypto_akcipher_verify_fuzz
 *
 * Fault injection. Spec 5.9.8.7 defines the akcipher verify opcode,
 * whose source carries both the message and the signature. A verify
 * naming an unknown session with an oversized source length must be
 * rejected without reading past the buffer or crashing the host. Post an
 * RSA verify on session zero with src_data_len 0xffffffff over a small
 * source and verify the host survives. The device outcome is up to it:
 * PASS, REJECT and WEDGED are all acceptable as long as the next test
 * can still run. Exercises the akcipher verify dispatch arm. Most
...
static test_result_t test_crypto_akcipher_verify_fuzz(struct virtio_dev *dev,
                                                      struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    dreq->u.akcipher.para.dst_data_len = 0;
    memset(src, 0x41, 16);
    inhdr->status = 0xFF;
    ...
    if (r == TEST_FAIL)
        TFAIL("device reported failure on a malformed akcipher verify");
    if (r == TEST_PASS || r == TEST_WEDGED)
        return r;
    TREJECT("no device response within timeout");
}

REGISTER_TEST(CR0057, VIRTIO_PCI_DEVICE_CRYPTO,
              test_crypto_akcipher_verify_fuzz,
              "A malformed akcipher verify keeps the host alive",
