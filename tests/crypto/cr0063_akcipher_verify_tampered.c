...
/*
 * CR0063: crypto_akcipher_verify_tampered
 *
 * Spec 5.9.10: RSA verify must reject a signature that does not match
 * the data. Sign a digest under the private key, flip a byte of the
 * signature, then verify it under the public key and expect the device
 * to reject it rather than accept it. A device that returns OK for a
 * tampered signature has a real verification flaw. Skips on Cloud
 * Hypervisor, when the akcipher service is not advertised, when RSA is
 * not advertised, and when the backend cannot perform PKCS#1 RSA; passes
 * on a QEMU built with a crypto library that provides RSA, for example
 * gcrypt.
 */
...
static test_result_t test_crypto_verify_tampered(struct virtio_dev *dev,
                                                 struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    sig[RSA_KEY_BYTES / 2] ^= 0xff;

    /* Verify the tampered signature under the public key. */
    memset(dreq, 0, sizeof(*dreq));
    ...
    if (inhdr->status == VIRTIO_CRYPTO_NOTSUPP || inhdr->status == VIRTIO_CRYPTO_ERR)
        return TEST_SKIP;
    if (inhdr->status == VIRTIO_CRYPTO_OK)
        TFAIL("device accepted a tampered signature");

    return TEST_PASS;
}

REGISTER_TEST(CR0063, VIRTIO_PCI_DEVICE_CRYPTO, test_crypto_verify_tampered,
              "An RSA verify rejects a tampered signature",
