...
/*
 * CR0019: crypto_encrypt_srclen_overflow
 *
 * Fault injection. Spec 5.9.8 carries the source length in the cipher
 * data request. A request whose source length is enormous while the
 * source descriptor is small must not drive the device to read past the
 * buffer, overflow its length arithmetic, or crash the host. Post an
 * encrypt with src_data_len 0xffffffff over a 16 byte source and verify
...
static test_result_t test_crypto_srclen_overflow(struct virtio_dev *dev,
                                                 struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    if (r == TEST_FAIL)
        TFAIL("device reported failure on an oversized source length");
    if (r == TEST_PASS || r == TEST_WEDGED)
        return r;
    TREJECT("no device response within timeout");
}

REGISTER_TEST(CR0019, VIRTIO_PCI_DEVICE_CRYPTO,
              test_crypto_srclen_overflow,
              "Encrypt with an oversized source length keeps the host alive",
