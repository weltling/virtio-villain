...
/*
 * CR0035: crypto_indirect_encrypt
 *
 * Spec 2.7.5.3 lets a driver deliver a whole request through one
 * indirect descriptor table. Feed an encrypt request to the data queue
 * as a legal five entry indirect table and verify the device processes
 * it end to end without crashing the host. With no session created the
 * device is expected to reject the op itself, so PASS, REJECT and WEDGED
 * are all acceptable as long as the next test can still run. Exercises
 * the indirect path on the crypto data queue. Skips on Cloud Hypervisor,
 * when the cipher service is not advertised, and when indirect
 * descriptors are not negotiated.
 */
...
static test_result_t test_crypto_indirect_encrypt(struct virtio_dev *dev,
                                                  struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    if (r == TEST_FAIL)
        TFAIL("device reported failure on an indirect encrypt request");
    if (r == TEST_PASS || r == TEST_WEDGED)
        return r;
    TREJECT("no device response within timeout");
}

REGISTER_TEST_REQUIRES(CR0035, VIRTIO_PCI_DEVICE_CRYPTO,
                       test_crypto_indirect_encrypt,
                       "An indirect encrypt request keeps the host alive",
