...
/*
 * CR0030: crypto_encrypt_readonly_dst
 *
 * Fault injection. Spec 5.9.8 has the device write ciphertext into a
 * device writable destination descriptor. A driver that marks the
 * destination read only lies about the buffer direction. The device must
 * reject the request or refuse to write rather than corrupt guest memory
 * or crash the host. Post an encrypt whose destination descriptor lacks
 * the write flag and verify the host survives. The device outcome is up
...
static test_result_t test_crypto_readonly_dst(struct virtio_dev *dev,
                                              struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    vring_raw_set_desc(vr, 2, vv_virt_to_phys(src), 16,
                       VRING_DESC_F_NEXT, 3);
    /* Destination descriptor deliberately read only. */
    ...
    if (r == TEST_FAIL)
        TFAIL("device reported failure on a read only destination");
    if (r == TEST_PASS || r == TEST_WEDGED)
        return r;
    TREJECT("no device response within timeout");
}

REGISTER_TEST(CR0030, VIRTIO_PCI_DEVICE_CRYPTO,
              test_crypto_readonly_dst,
              "A read only destination descriptor keeps the host alive",
