...
/*
 * CR0010: crypto_cipher_iv_changes_output
 *
 * Spec 5.9.8: for a CBC mode cipher the IV is mixed into the first
 * block, so encrypting the same plaintext under the same key with two
 * different IVs must produce different ciphertext. Create an AES-CBC
 * encrypt session and encrypt one 16 byte block twice with distinct
 * IVs, then verify the two ciphertexts differ. Skips on Cloud
...
static test_result_t test_crypto_cipher_iv_changes(struct virtio_dev *dev,
                                                   struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    uint8_t iv_a[16], iv_b[16], plaintext[16], ct_a[16], ct_b[16];
    memset(iv_a, 0x00, sizeof(iv_a));
    memset(iv_b, 0xFF, sizeof(iv_b));
    memset(plaintext, 0x41, sizeof(plaintext));

    r = encrypt_block(dev, vr, 0, session_id, iv_a, plaintext, ct_a);
    if (r != TEST_PASS)
        return r;
    r = encrypt_block(dev, vr, 1, session_id, iv_b, plaintext, ct_b);
    if (r != TEST_PASS)
        return r;

    if (memcmp(ct_a, ct_b, 16) == 0)
        TFAIL("different IVs produced identical ciphertext");
...
REGISTER_TEST(CR0010, VIRTIO_PCI_DEVICE_CRYPTO,
              test_crypto_cipher_iv_changes,
              "Cipher CBC output changes with the IV",
