...
/*
 * CR0038: crypto_encrypt_zero_lengths
 *
 * Fault injection. Spec 5.9.8 carries the IV, source and destination
 * lengths in the cipher data request. A request that declares all three
 * as zero is degenerate and must be handled without dividing by a block
 * size of zero, looping forever, or crashing the host. Post an encrypt
 * with iv_len, src_data_len and dst_data_len all zero and verify the
 * host survives. The device outcome is up to it: PASS, REJECT and WEDGED
 * are all acceptable as long as the next test can still run. Most
 * valuable under an ASan VMM. Skips on Cloud Hypervisor and when the
 * cipher service is not advertised.
 */
...
static test_result_t test_crypto_zero_lengths(struct virtio_dev *dev,
                                              struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    dreq->u.sym_cipher.para.src_data_len = 0;
    ...
    if (r == TEST_FAIL)
        TFAIL("device reported failure on an all zero length request");
    if (r == TEST_PASS || r == TEST_WEDGED)
        return r;
    TREJECT("no device response within timeout");
}

REGISTER_TEST(CR0038, VIRTIO_PCI_DEVICE_CRYPTO,
              test_crypto_zero_lengths,
              "An all zero length encrypt keeps the host alive",
