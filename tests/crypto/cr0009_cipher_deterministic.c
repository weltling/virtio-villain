...
/*
 * CR0009: crypto_cipher_deterministic
 *
 * Spec 5.9.8: a symmetric cipher operation is a pure function of the
 * session key, the IV and the input, so encrypting the same block twice
 * with the same key and IV must yield the same ciphertext. Create an
 * AES-CBC encrypt session, encrypt one 16 byte block twice on a data
 * queue with an identical IV, and verify both results match each other
 * and differ from the plaintext. Skips on Cloud Hypervisor, when the
 * cipher service or AES-CBC is not advertised, and when the backend
 * cannot perform the algorithm; passes under a QEMU with a crypto
 * library.
 */
...
static test_result_t test_crypto_cipher_deterministic(struct virtio_dev *dev,
                                                      struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    uint8_t iv[16], plaintext[16], ct1[16], ct2[16];
    memset(iv, 0x00, sizeof(iv));
    memset(plaintext, 0x41, sizeof(plaintext));

    r = encrypt_block(dev, vr, 0, session_id, iv, plaintext, ct1);
    if (r != TEST_PASS)
        return r;
    r = encrypt_block(dev, vr, 1, session_id, iv, plaintext, ct2);
    if (r != TEST_PASS)
        return r;

    if (memcmp(ct1, plaintext, 16) == 0)
        TFAIL("ciphertext equals plaintext, no encryption happened");
    if (memcmp(ct1, ct2, 16) != 0)
        TFAIL("same key, IV and input produced differing ciphertext");
...
REGISTER_TEST(CR0009, VIRTIO_PCI_DEVICE_CRYPTO,
              test_crypto_cipher_deterministic,
              "Cipher encrypt is deterministic for a fixed key and IV",
