...
/*
 * CR0003: crypto_cipher_encrypt_data
 *
 * Spec 5.9.8: after creating a symmetric cipher session on the control
 * queue the driver submits an encrypt request on a data queue with the
 * IV, source and destination buffers. Create an AES-CBC encrypt
 * session, encrypt one 16 byte block, verify the device acks OK and
 * that the ciphertext differs from the plaintext, then destroy the
 * session. Skips on Cloud Hypervisor, when the cipher service or
 * AES-CBC is not advertised, and when the backend cannot perform the
 * algorithm; passes under a QEMU with a crypto library.
 */
...
static test_result_t test_crypto_cipher_encrypt(struct virtio_dev *dev,
                                                struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    /* Create an AES-CBC encrypt session on the control queue. */
    ...
    /* Encrypt one block on the data queue. */
...
REGISTER_TEST(CR0003, VIRTIO_PCI_DEVICE_CRYPTO, test_crypto_cipher_encrypt,
              "Cipher encrypt on the data queue transforms the block",
