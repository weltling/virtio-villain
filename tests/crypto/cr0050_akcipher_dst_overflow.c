...
/*
 * CR0050: crypto_akcipher_dst_overflow
 *
 * Fault injection. Spec 5.9.8.7 carries the destination length in the
 * akcipher data request. A request whose destination length is enormous
 * while the destination descriptor is small must not drive the device to
 * write past the buffer. This is the write side companion to the source
 * length case. Post an RSA encrypt on the data queue with dst_data_len
 * 0xffffffff over a 16 byte destination and verify the host survives.
 * The device outcome is up to it: PASS, REJECT and WEDGED are all
 * acceptable as long as the next test can still run. Exercises the
 * akcipher data dispatch. Most valuable under an ASan VMM with akcipher
...
static test_result_t test_crypto_akcipher_dst_overflow(struct virtio_dev *dev,
                                                       struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    dreq->u.akcipher.para.dst_data_len = 0xffffffffu;
    memset(src, 0x41, 16);
    memset(dst, 0, 16);
    inhdr->status = 0xFF;
    ...
    if (r == TEST_FAIL)
        TFAIL("device reported failure on an oversized akcipher destination");
    if (r == TEST_PASS || r == TEST_WEDGED)
        return r;
    TREJECT("no device response within timeout");
}

REGISTER_TEST(CR0050, VIRTIO_PCI_DEVICE_CRYPTO,
              test_crypto_akcipher_dst_overflow,
              "An oversized akcipher destination length keeps the host alive",
