...
/*
 * CR0023: crypto_encrypt_dstlen_short
 *
 * Fault injection. Spec 5.9.8 carries separate source and destination
 * lengths in the cipher data request. A request whose destination
 * length is smaller than the source, backed by a short destination
 * buffer, must not drive the device to write past that buffer or crash
 * the host. Post an encrypt with src_data_len 16 but dst_data_len 4 over
 * a 4 byte destination and verify the host survives. The device outcome
...
static test_result_t test_crypto_dstlen_short(struct virtio_dev *dev,
                                              struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    dreq->u.sym_cipher.para.dst_data_len = 4;
    ...
    vring_raw_set_desc(vr, 2, vv_virt_to_phys(src), 16,
                       VRING_DESC_F_NEXT, 3);
    vring_raw_set_desc(vr, 3, vv_virt_to_phys(dst), 4,
    ...
    if (r == TEST_FAIL)
        TFAIL("device reported failure on a short destination length");
    if (r == TEST_PASS || r == TEST_WEDGED)
        return r;
    TREJECT("no device response within timeout");
}

REGISTER_TEST(CR0023, VIRTIO_PCI_DEVICE_CRYPTO,
              test_crypto_dstlen_short,
              "Encrypt with a short destination length keeps the host alive",
