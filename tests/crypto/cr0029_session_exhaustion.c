/* SPDX-License-Identifier: Apache-2.0 */
/*
 * CR0029: crypto_session_exhaustion
 *
 * Fault injection. Spec 5.9.7 lets the driver create cipher sessions on
 * the control queue. A device keeps per session state and must bound the
 * number of live sessions rather than overrun its table or crash. Create
 * many sessions in a row without destroying any, past a typical backend
 * cap, and verify the host survives. Once the cap is reached the device
 * is expected to reject further creates gracefully; either way the run
 * must continue. Most valuable under an ASan VMM. Skips on Cloud
 * Hypervisor and when the cipher service is not advertised.
 */
#include "tests/test.h"
#include "lib/util.h"
#include "lib/vring.h"
#include "lib/virtio_pci.h"
#include "lib/virtio_spec.h"

#include <string.h>

#define CRYPTO_SESSION_ATTEMPTS 300

static test_result_t test_crypto_session_exhaustion(struct virtio_dev *dev,
                                                    struct vring *vr)
{
    (void)vr;
    if (!dev->device_cfg)
        return TEST_SKIP;
    volatile struct virtio_crypto_config *cfg =
        (volatile struct virtio_crypto_config *)dev->device_cfg;
    if (!(cfg->crypto_services & (1u << VIRTIO_CRYPTO_SERVICE_CIPHER)))
        return TEST_SKIP;

    struct vring cvr;
    vring_alloc(&cvr, 16);
    vring_attach(dev, &cvr, (uint16_t)cfg->max_dataqueues);

    struct virtio_crypto_op_ctrl_req *req = vv_alloc_pages(1);
    uint8_t *key = vv_alloc_pages(1);
    struct virtio_crypto_session_input *sin = vv_alloc_pages(1);
    memset(key, 0x2b, 16);

    for (int i = 0; i < CRYPTO_SESSION_ATTEMPTS; i++) {
        memset(req, 0, sizeof(*req));
        req->header.opcode = VIRTIO_CRYPTO_CIPHER_CREATE_SESSION;
        req->header.algo = VIRTIO_CRYPTO_CIPHER_AES_CBC;
        req->u.sym_create.para.algo = VIRTIO_CRYPTO_CIPHER_AES_CBC;
        req->u.sym_create.para.keylen = 16;
        req->u.sym_create.para.op = VIRTIO_CRYPTO_OP_ENCRYPT;
        req->u.sym_create.op_type = VIRTIO_CRYPTO_SYM_OP_CIPHER;
        memset(sin, 0, sizeof(*sin));
        sin->status = 0xFF;

        uint16_t slot = (uint16_t)(i % 16);
        vring_raw_set_desc(&cvr, 0, vv_virt_to_phys(req), sizeof(*req),
                           VRING_DESC_F_NEXT, 1);
        vring_raw_set_desc(&cvr, 1, vv_virt_to_phys(key), 16,
                           VRING_DESC_F_NEXT, 2);
        vring_raw_set_desc(&cvr, 2, vv_virt_to_phys(sin), sizeof(*sin),
                           VRING_DESC_F_WRITE, 0);
        vring_raw_set_avail(&cvr, slot, 0);
        vring_raw_set_avail_idx(&cvr, (uint16_t)(i + 1));

        test_result_t r = vv_kick_and_wait(dev, &cvr, 0, VV_TIMEOUT_MS);
        if (r == TEST_FAIL)
            TFAIL("device reported failure while creating session %d", i);
        if (r == TEST_WEDGED)
            return TEST_WEDGED;
        if (r != TEST_PASS)
            break;
    }

    return TEST_PASS;
}

REGISTER_TEST(CR0029, VIRTIO_PCI_DEVICE_CRYPTO,
              test_crypto_session_exhaustion,
              "Many session creates without destroy keep the host alive",
              VIRTIO_SPEC_V1_2, "5.9.7");
