...
/*
 * CR0013: crypto_cipher_multiblock
 *
 * Spec 5.9.8: a cipher request may carry more than one block. In CBC
 * mode each block is chained through the previous ciphertext, so
 * encrypting two identical plaintext blocks yields two differing
 * ciphertext blocks. Create an AES-CBC encrypt session, encrypt a 32
 * byte input of two identical blocks on a data queue, and verify the
 * device acks OK, the ciphertext differs from the plaintext, and the
 * two ciphertext blocks differ from each other. Skips on Cloud
 * Hypervisor, when the cipher service or AES-CBC is not advertised, and
 * when the backend cannot perform the algorithm; passes under a QEMU
...
static test_result_t test_crypto_cipher_multiblock(struct virtio_dev *dev,
                                                   struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    /* Encrypt two identical 16 byte blocks on the data queue. */
    ...
    dreq->u.sym_cipher.para.src_data_len = 32;
    dreq->u.sym_cipher.para.dst_data_len = 32;
    ...
    vring_raw_set_desc(vr, 1, vv_virt_to_phys(iv), 16,
                       VRING_DESC_F_NEXT, 2);
    vring_raw_set_desc(vr, 2, vv_virt_to_phys(src), 32,
                       VRING_DESC_F_NEXT, 3);
    vring_raw_set_desc(vr, 3, vv_virt_to_phys(dst), 32,
    ...
    if (memcmp(dst, src, 32) == 0)
        TFAIL("ciphertext equals plaintext, no encryption happened");
    if (memcmp(dst, dst + 16, 16) == 0)
        TFAIL("identical plaintext blocks gave identical ciphertext, "
              "CBC chaining absent");
...
REGISTER_TEST(CR0013, VIRTIO_PCI_DEVICE_CRYPTO,
              test_crypto_cipher_multiblock,
              "Cipher encrypts a multi block request with CBC chaining",
