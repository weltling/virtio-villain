...
/*
 * CR0028: crypto_encrypt_dstlen_overflow
 *
 * Fault injection. Spec 5.9.8 carries the destination length in the
 * cipher data request. A request whose destination length is enormous
 * while the destination descriptor is small must not drive the device to
 * write past the buffer, overflow its length arithmetic, or crash the
 * host. This is the write side companion to the source length case. Post
 * an encrypt with dst_data_len 0xffffffff over a 16 byte destination and
 ...
 * Most valuable under an ASan VMM. Skips on Cloud Hypervisor and when the
...
static test_result_t test_crypto_dstlen_overflow(struct virtio_dev *dev,
                                                 struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    dreq->u.sym_cipher.para.dst_data_len = 0xffffffffu;
    ...
    if (r == TEST_FAIL)
        TFAIL("device reported failure on an oversized destination length");
    if (r == TEST_PASS || r == TEST_WEDGED)
        return r;
    TREJECT("no device response within timeout");
}

REGISTER_TEST(CR0028, VIRTIO_PCI_DEVICE_CRYPTO,
              test_crypto_dstlen_overflow,
              "Encrypt with an oversized destination length keeps the host alive",
