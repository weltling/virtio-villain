...
/*
 * CR0043: crypto_akcipher_keylen_overflow
 *
 * Fault injection. Spec 5.9.10 carries the key length in the akcipher
 * create session request. A key length of 0xffffffff over a small key
 * descriptor must be bounded against the advertised max size rather than
 * driving a huge allocation or a read past the buffer. Post an RSA
 * create session with keylen 0xffffffff over a 16 byte key and verify
...
static test_result_t test_crypto_akcipher_keylen_overflow(struct virtio_dev *dev,
                                                          struct vring *vr)
{
    (void)vr;
    if (!dev->device_cfg)
    ...
    req->u.akcipher_create.para.keylen = 0xffffffffu;
    ...
    if (r == TEST_FAIL)
        TFAIL("device reported failure on an oversized akcipher key length");
    if (r == TEST_PASS || r == TEST_WEDGED)
        return r;
    TREJECT("no device response within timeout");
}

REGISTER_TEST(CR0043, VIRTIO_PCI_DEVICE_CRYPTO,
              test_crypto_akcipher_keylen_overflow,
              "An oversized akcipher key length keeps the host alive",
