...
/*
 * CR0027: crypto_hash_sha1
 *
 * Spec 5.9.9: with the hash service the driver creates a hash session
 * and submits data on a data queue to receive a digest. Create a SHA1
 * session, hash a 16 byte input, verify the device acks OK and writes a
 * non zero 20 byte digest, then destroy the session. Skips on Cloud
 * Hypervisor, when SHA1 is not advertised, and when the backend cannot
 * perform it; passes under a QEMU with a crypto library, where the
 * builtin backend advertises SHA1.
 */
...
static test_result_t test_crypto_hash_sha1(struct virtio_dev *dev,
                                           struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    if (!(cfg->crypto_services & (1u << VIRTIO_CRYPTO_SERVICE_HASH)))
        return TEST_SKIP;
    if (!(cfg->hash_algo & (1u << VIRTIO_CRYPTO_HASH_SHA1)))
        return TEST_SKIP;
    ...
    /* Create a SHA1 hash session on the control queue. */
    ...
    creq->header.algo = VIRTIO_CRYPTO_HASH_SHA1;
    creq->u.hash_create.para.algo = VIRTIO_CRYPTO_HASH_SHA1;
    creq->u.hash_create.para.hash_result_len = 20;
    ...
    dreq->header.algo = VIRTIO_CRYPTO_HASH_SHA1;
    dreq->header.session_id = session_id;
    dreq->u.hash.para.src_data_len = 16;
    dreq->u.hash.para.hash_result_len = 20;
    memset(src, 0x61, 16);
    memset(digest, 0, 20);
    inhdr->status = 0xFF;
    ...
    vring_raw_set_desc(vr, 1, vv_virt_to_phys(src), 16,
                       VRING_DESC_F_NEXT, 2);
    vring_raw_set_desc(vr, 2, vv_virt_to_phys(digest), 20,
...
REGISTER_TEST(CR0027, VIRTIO_PCI_DEVICE_CRYPTO, test_crypto_hash_sha1,
              "SHA1 hash op produces a non zero digest",
