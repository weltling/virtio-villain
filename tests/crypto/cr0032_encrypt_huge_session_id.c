...
/*
 * CR0032: crypto_encrypt_huge_session_id
 *
 * Fault injection. Spec 5.9.8 selects the session for a data op by
 * session id. A request naming a session id at the top of the 64 bit
 * range must be bounds checked rather than used as a raw table index.
 * Post an encrypt with session_id 0xffffffffffffffff and verify the host
...
static test_result_t test_crypto_huge_session_id(struct virtio_dev *dev,
                                                 struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    dreq->header.session_id = 0xffffffffffffffffULL;
    ...
    if (r == TEST_FAIL)
        TFAIL("device reported failure on a huge session id");
    if (r == TEST_PASS || r == TEST_WEDGED)
        return r;
    TREJECT("no device response within timeout");
}

REGISTER_TEST(CR0032, VIRTIO_PCI_DEVICE_CRYPTO,
              test_crypto_huge_session_id,
              "An out of range session id keeps the host alive",
