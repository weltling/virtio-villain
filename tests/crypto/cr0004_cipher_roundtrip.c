...
/*
 * CR0004: crypto_cipher_roundtrip
 *
 * Spec 5.9.8: encrypting a block then decrypting the ciphertext with a
 * matching key must recover the original plaintext. Create an AES-CBC
 * encrypt session and a decrypt session with the same key and IV,
 * encrypt a 16 byte block on a data queue, decrypt the ciphertext, and
 * verify the result equals the plaintext. Skips on Cloud Hypervisor,
 * when AES-CBC is not advertised, and when the backend cannot perform
 * the algorithm; passes under a QEMU with a crypto library.
 */
...
static test_result_t test_crypto_cipher_roundtrip(struct virtio_dev *dev,
                                                  struct vring *vr)
{
    if (!dev->device_cfg)
        return TEST_SKIP;
    ...
    if (memcmp(cipher, plain, 16) == 0)
        TFAIL("ciphertext equals plaintext");
    ...
    if (memcmp(recovered, plain, 16) != 0)
        TFAIL("decrypt did not recover the plaintext");

    return TEST_PASS;
}

REGISTER_TEST(CR0004, VIRTIO_PCI_DEVICE_CRYPTO, test_crypto_cipher_roundtrip,
              "Cipher encrypt then decrypt recovers the plaintext",
